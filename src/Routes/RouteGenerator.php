    ...
    public function resolve(array $metadata, array $config): RouteCollection
    {
        ...
        foreach ($resolvedEntities as $fqcn => $shortname) {
            foreach (Action::cases() as $action) {
                ...
                $routesCollection->add($routeName, new Route($action->routePattern($shortname), defaults: ['_controller' => $action->controller()], options: ['fqcn' => $fqcn,], methods: $action->httpMethods()));
            }
        }
