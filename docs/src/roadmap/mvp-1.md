...
<details class="k-ticket k-ticket--green">
  <summary>11. Named format registry <span class="k-status k-status--green">Ready</span></summary>

  <table class="k-ticket">
    <tbody>
      <tr><th>Existing.</th><td>Les formatteurs DateTime (<code>DateFormatter</code>, <code>TimeFormatter</code>, <code>DateTimeFormatter</code>) utilisent des longueurs ICU figées : date MEDIUM, heure SHORT, date-heure MEDIUM plus SHORT. Ces valeurs sont en dur dans chaque formatter. <code>FormattingContext</code> porte déjà <code>dateFormat</code>, <code>timeFormat</code> et <code>dateTimeFormat</code>, mais <code>PropertyAccessorExtension::context()</code> ne les injecte jamais : seuls <code>currency</code>, <code>ucfirst</code> et <code>maximum_fraction_digits</code> sont lus dans <code>formatterOptions</code>. La clé <code>datetime_format</code> n'existe pas dans le nœud <code>formatter_options</code> de <code>Configuration</code> : la config la rejette. Il n'existe aucun mécanisme de format nommé. Pour un format personnalisé, l'utilisateur doit écraser le template ou remplacer le formatteur entier via config.</td></tr>
      <tr><th>Expected.</th><td>Le hôte définit des formats nommés dans <code>karross.datetime_formats</code>. Une propriété les référence via <code>formatter_options.datetime_format</code>. Le bundle livre 12 formats fournis par défaut (<code>k_short_date</code> à <code>k_full_datetime</code>) qui pointent vers les longueurs ICU.<br><br>
      Toute la décision de format est prise au build. Les metadata de la propriété embarquent le résultat : soit une table de motifs (locale vers motif), soit des identifiants de longueur ICU. Au runtime, le rendu lit seulement ces données déjà en place et choisit l'entrée qui correspond à la locale de la requête. Aucun service de format nommé n'est appelé au rendu.<br><br>
      Exemple — format métier :
      <pre><code>karross:
  datetime_formats:
  ...
  entities:
    App\Entity\Article:
      ...
      Résultat : <code>createdAt</code> affiche <code>14 septembre 2026 à 15:30</code> en français, <code>September 14, 2026 at 15:30</code> en anglais, <code>2026-09-14 15:30</code> pour une locale sans entrée (repli sur <code>default</code>).<br><br>
      Exemple — personnaliser un format fourni par défaut :
      <pre><code>karross:
  datetime_formats:
    k_medium_date:
      fr: 'd MMMM yyyy'</code></pre>
      Toutes les propriétés date qui n'ont pas d'option <code>datetime_format</code> explicite utilisent <code>k_medium_date</code> par défaut. En français, elles s'affichent donc avec <code>14 septembre 2026</code>. Les autres locales gardent la longueur ICU MEDIUM.</td></tr>
      <tr><th>Plan.</th><td><ol>
        <li><strong>Nœud de config <code>datetime_formats</code></strong> (<code>src/DependencyInjection/Configuration.php</code>) : clé = nom du format, valeur = motif en chaîne unique <strong>ou</strong> tableau <code>locale =&gt; motif</code>. Les deux formes se normalisent au même format interne. Accesseur <code>datetime_formats()</code> sur <code>KarrossConfig</code> + mise à jour du docblock.</li>
        <li><strong>Table statique des 12 formats fournis par défaut</strong> (table littérale à côté du registre, pas de déduction par nom) : chaque entrée <code>k_{short|medium|long|full}_{date|time|datetime}</code> porte sa correspondance ICU. Pour <code>date</code> et <code>time</code> : une longueur. Pour <code>datetime</code> : une longueur de date et une longueur d'heure — <code>k_medium_datetime</code> mappe vers date MEDIUM et heure SHORT pour parité stricte avec le défaut actuel de <code>DateTimeFormatter</code>.</li>
        <li><strong><code>NamedDatetimeFormats</code>, outil de build uniquement</strong> (<code>src/Formatters/Options/NamedDatetimeFormats.php</code>) : renvoie la config d'un nom de format. Priorité à la définition du hôte pour ce nom, sinon au format fourni par défaut, sinon nom inconnu. Pas de tag DI, pas d'injection dans <code>PropertyAccessorExtension</code> ni dans aucun service de rendu.</li>
        <li><strong>Clé <code>datetime_format</code> sur la propriété</strong> : <code>scalarNode('datetime_format')</code> sous <code>formatter_options</code> dans <code>Configuration.php</code>. Sans ce nœud, la config rejette la clé aujourd'hui.</li>
        <li><strong>Écriture dans les metadata, dans <code>EntityMetadataBuilder</code></strong> : pour chaque propriété dont le formatter résolu est <code>DateFormatter</code>, <code>TimeFormatter</code> ou <code>DateTimeFormatter</code> :
          <ul>
            <li>nom à résoudre = <code>formatterOptions['datetime_format']</code> s'il est présent, sinon le nom implicite du formatter (<code>k_medium_date</code> pour la date, <code>k_short_time</code> pour l'heure, <code>k_medium_datetime</code> pour la date-heure) ;</li>
            <li>nom explicite inconnu → <code>\InvalidArgumentException</code> au build, message indiquant le nom et la propriété ;</li>
            <li>résolution via le registre, puis écriture dans <code>formatterOptions['datetime_format']</code> sous forme de tableau résolu : <code>['patterns' =&gt; ['fr' =&gt; 'd MMMM yyyy', ...]]</code> pour une table de motifs, ou <code>['presets' =&gt; ['date' =&gt; 'medium']]</code> pour une longueur ICU, ou les deux si la définition fournie par le hôte ne couvre pas toutes les locales ; la clé de config (chaîne) est remplacée par ce tableau au build ;</li>
          </ul>
          C'est cette étape qui rend effective la personnalisation de <code>k_medium_date</code> : le builder interroge le nom implicite de chaque propriété date, même sans option <code>datetime_format</code>.</li>
        <li><strong>Choix de locale au rendu, dans <code>PropertyAccessorExtension::context()</code></strong> : lecture de <code>formatterOptions['datetime_format']</code> depuis les metadata (aucune dépendance au registre). Repli dans l'ordre : locale exacte de la requête, puis langue seule, puis <code>default</code>. Si aucune clé de <code>patterns</code> ne correspond, la longueur de <code>presets</code> s'applique, sinon le défaut du formatter. Injection dans le slot du formatter : <code>dateFormat</code> pour <code>DateFormatter</code>, <code>timeFormat</code> pour <code>TimeFormatter</code>, <code>dateTimeFormat</code> pour <code>DateTimeFormatter</code>.</li>
        <li><strong><code>FormattingContext</code> et les trois formatters DateTime</strong> : le contexte gagne <code>dateFormatPreset</code> et <code>timeFormatPreset</code> (longueurs ICU). <code>DateTimeFormatter</code> lit les deux — sa partie date et sa partie heure — donc pas de troisième champ <code>dateTimeFormatPreset</code>. Chaque <code>resolvePattern()</code> applique la priorité : motif présent, puis longueur présente, puis défaut actuel inchangé. Sans ce palier, un <code>k_short_date</code> non surchargé resterait bloqué sur MEDIUM.</li>
        <li><strong>Tests</strong> :
          <ul>
            <li>build : propriété avec <code>datetime_format: my_custom_datetime_format</code> embarque <code>formatterOptions['datetime_format']['patterns']</code> avec les trois clés ; propriété date sans option embarque <code>['presets' =&gt; ['date' =&gt; 'medium']]</code> (nom implicite <code>k_medium_date</code>) ; la définition du hôte pour <code>k_medium_date</code> remplace l'embarquement de toutes les propriétés date sans <code>datetime_format</code> ; nom inconnu lève l'exception au build ; forme chaîne et forme tableau de <code>datetime_formats</code> acceptées ;</li>
            <li>runtime : locale <code>fr</code> choisit la clé <code>fr</code>, locale <code>fr_CA</code> retombe sur <code>fr</code> puis sur <code>default</code> ;</li>
            <li>E2E + property-based (BlackBox) : page index en français et en anglais avec une propriété date en format personnalisé, plus une app configurée qui personnalise <code>k_medium_date</code> ; génération de combinaisons locales × noms de formats (fournis par défaut et personnalisés) pour couvrir l'espace, seed fixe, shrinking des échecs.</li>
          </ul></li>
        <li><strong>Doc</strong> : section « Named date/time formats » dans <code>docs/src/customization/formatting-values.md</code> — table des 12 formats fournis par défaut, exemple <code>my_custom_datetime_format</code>, exemple de personnalisation de <code>k_medium_date</code>, clé <code>datetime_format</code> ajoutée au tableau de <code>formatter_options</code>.</li>
      </ol></td></tr>
    </tbody>
  </table>
</details>
