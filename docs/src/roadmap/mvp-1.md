...
<details class="k-ticket k-ticket--green">
  <summary>Collect &amp; Computed <span class="k-status k-status--green">Ready</span></summary>
  ...
  <table class="k-ticket">
    <tbody>
      ...
      <ul>
        ...
        <li>les <strong>objets construits</strong> (<code>Metadata\Computed</code>) : le <code>PropertyMetadata</code> porte ces faits, <em>et les <strong>déductions</strong> utiles au bundle, toutes calculées au build à partir des faits, en <strong>une seule passe</strong> sur les <code>ClassMetadata</code> de Doctrine</em> — le formatter, <code>typeHierarchy</code>, et demain le widget. <code>typeHierarchy</code> est la <strong>chaîne ordonnée des noms de types</strong> (du plus spécifique au plus générique, paliers « famille » inclus — ex. PHP <code>int</code> + Doctrine <code>bigint</code> → <code>[bigint, int, number]</code>, PHP <code>string</code> + Doctrine <code>blob</code> → <code>[blob, string]</code>), <strong>indépendante de l'action</strong> : aucune dimension template (action × propriété × entité) dans la metadata. <strong>Aucun consommateur ne re-déduit de sémantique au rendu</strong> : <code>TemplateResolver</code> <em>compose</em> les candidats à partir de <code>typeHierarchy</code> + le contexte (action/propriété/entité) et ne tranche que l'<em>existence physique</em> des fichiers (<code>resolveTemplate()</code> + cache, ticket 2). Chaque déduction est décidée par son propre mécanisme d'usage, remplaçable par la config hôte ;</li>
...
Règles de remplissage des faits (posées ici une fois pour toutes) :
      ...
      <tr><th>Plan.</th><td><ol><li><strong>Dépendances (décision REC 2026-09-13)</strong> : <code>doctrine/orm</code> et <code>doctrine/dbal</code> passent en <code>require</code> (le bundle cible Symfony + Doctrine ORM — l'hôte les fournit déjà ; <code>Doctrine\DBAL\Types\Types</code> est déjà importé en <code>src/</code>, fuite à régulariser). Le collecteur est <strong>typé contre <code>Doctrine\ORM\Mapping\ClassMetadata</code></strong>, pas l'interface persistence : les details de colonne y sont lisibles via <code>getFieldMapping()</code> (nullable, length, precision, scale, <code>enumType</code>…) — <code>unsigned</code>, <code>fixed</code> et <code>default</code> vivent dans <code>options</code> du <code>FieldMapping</code>, lus par le collecteur ; aucun <code>instanceof</code>-downcast dans le collecteur lui-même — le builder fait une <strong>assertion runtime unique</strong> en haut de sa boucle (<code>getAllMetadata()</code> retourne l'interface persistence, l'instance réelle est celle de l'ORM : garantie ciblée puisque <code>doctrine/orm</code> passe en <code>require</code>), PHPStan max tranquille. La note tests.md (« src ne consomme que doctrine/persistence ») devient « ne manipule jamais les proxy ; la lecture metadata est typée ORM ».</li><li>Collecteur : réécrire <code>PropertyTypeDetector</code> en collecteur de faits (<code>Metadata\Collect</code>) — reçoit la propriété + le mapping complet (<code>getFieldMapping()</code>/<code>getAssociationMapping()</code>, passés par <code>EntityMetadataBuilder::buildFields()</code>/<code>buildAssociations()</code>) ; aucune projection, suppression de la branche PHPDoc. Cardinalité = <strong>réponse directe de Doctrine</strong> (<code>isSingleValuedAssociation()</code>/<code>isCollectionValuedAssociation()</code>), plus de devinette PHP/PHPDoc.</li><li>Computed : <code>PropertyMetadata</code> porte les faits (phpType, doctrineType, details, cardinalité, conflit) <em>et les déductions</em> — <code>formatter</code> (déjà présent), <code>typeHierarchy</code>, et la place du futur <code>widget</code> ; suppression du champ <code>type</code>.</li><li>Usages : <code>FormatterResolver</code> devient une chaîne qui lit les faits (défauts déterministes par type PHP/Doctrine, la famille <code>number</code> pouvant servir de racine commune int/float) ; <strong>règle socle (non-régression du rendu E2E) : PHP gagne quand il existe, <em>sauf</em> la famille datetime raffinée par Doctrine</strong> (date/time/datetime — <code>DateTimeImmutable</code>+<code>date_immutable</code> → Date, pas DateTime) ; les paires en conflit sont <strong>flagées, pas tranchées</strong> (PHP gagne au socle, ex. <code>?string</code>+<code>decimal</code> → String). <code>typeHierarchy</code> (chaîne ordonnée, action-indépendante) est déduite au build à partir des faits (<code>[doctrineType?, phpType?, famille?]</code>, seule famille <code>number</code> int/float au socle) et ne sert QUE <code>TemplateResolver</code> — le formatter lit les faits directement, hiérarchie et formatter restent <strong>découplés</strong> ; <code>TemplateResolver</code> <em>compose</em> les candidats (action × propriété × entité × <code>typeHierarchy</code>, les motifs <code>_type_{...}</code> s'expansent sur la chaîne spécifique→générique) et se réduit à la vérification d'existence des fichiers (<code>resolveTemplate()</code>) + cache — aucun template <code>_type_*</code> existant à casser. <strong>Précision de périmètre (décision REC 2026-09-14)</strong> : cette étape pose le <strong>principe</strong> (le mécanisme de la hiérarchie par <code>typeHierarchy</code>) sans aller plus loin — pas d'inventaire exhaustif des cas de surcharge de template selon les metadata ; les cas d'usage seront documentés à la demande (voir mémoire).</li><li>Enum : suppression totale de <code>PropertyType</code>, plus aucun consommateur au build ni au rendu.</li><li>Adressage : read-models → <code>Metadata\Computed</code>, machines → <code>Metadata\Collect</code> (mouvement du ticket 5) ; <code>services.php</code> + <code>EntityMetadataRegistry</code> à jour.</li><li>Tests : règles de fusion (typé / non typé → Doctrine / conflit → drapeau), contenu des faits et des déductions (formatter, <code>typeHierarchy</code>), fidélité cache dev/prod (ticket 2).</li></ol></td></tr>
    </tbody>
  </table>
</details>

<details class="k-ticket k-ticket--red">
  <summary>Conflict flag — pair table <span class="k-status k-status--red">Proposed</span></summary>

  <table class="k-ticket">
    <tbody>
      <tr><th>Existing.</th><td>Les faits (phpType × doctrineType) peuvent se contredire — ex. PHP <code>string</code>, Doctrine <code>decimal</code> (<code>Article::$price</code>) — sans règle formelle pour départager ; le collecteur doit promettre ce comportement avant que les tickets de cas n'y répondent.</td></tr>
      <tr><th>Expected.</th><td>Le collecteur log toutes les paires <code>(phpType × doctrineType)</code> rencontrées pendant MVP-1 et leur statut (accord / conflit à trancher) ; le drapeau « conflit » porté par <code>PropertyMetadata</code> signale les paires non stabilisées ; chacune est tranchée par le ticket du cas concerné.</td></tr>
      <tr><th>Prerequisites.</th><td>Collect &amp; Computed.</td></tr>
      <tr><th>Analysis.</th><td>Table des paires à tenir à jour au fil des tickets de cas (ex. <code>?string</code>+<code>decimal</code> = conflit, <code>DateTimeInterface</code>+<code>datetime</code> = raffinement sans conflit). Le connecteur = le drapeau ; la résolution = le ticket de cas.</td></tr>
    </tbody>
  </table>
</details>

<details class="k-ticket k-ticket--red">
  <summary>Cardinality fact — carrier without consumers <span class="k-status k-status--red">Proposed</span></summary>

  <table class="k-ticket">
    <tbody>
      <tr><th>Existing.</th><td><code>AssociationMetadata</code> existe mais la cardinalité (to-one/to-many) n'est pas portée comme un fait distinct ; les consommateurs (templates item, foncier multipass) devinent ou s'appuient sur le premier membre d'un enum.</td></tr>
      <tr><th>Expected.</th><td>Le fait cardinalité (<code>to-one</code>/<code>to-many</code>) vit sur <code>PropertyMetadata</code>, rempli directement par <code>isSingleValuedAssociation()</code>/<code>isCollectionValuedAssociation()</code> ; carré et marge, sans autre consommateur dans le socle — les consommateurs atterrissent dans les tickets Association to-one/to-many.</td></tr>
      <tr><th>Prerequisites.</th><td>Collect &amp; Computed.</td></tr>
      <tr><th>Analysis.</th><td>Carrier sans consumer → le socle l'expose, les tickets de cas l'exploitent. Porté par <code>PropertyMetadata</code> pour que la lecture reste uniforme entre champ et association.</td></tr>
    </tbody>
  </table>
</details>
...
<details class="k-ticket k-ticket--red">
  ...
  <table class="k-ticket">
    <tbody>
      ...
      <tr><th>Expected.</th><td>Tous les cas Boolean (typé, nullable, union avec null, non typé, conflit Doctrine, embedded) lisent les mêmes faits (<code>phpType</code> bool, <code>doctrineType</code> boolean) ; formatteur <code>TrueFalseFormatter</code> ; widget futur = checkbox + <code>required</code> ; <code>typeHierarchy</code> = [bool] (Doctrine boolean n'apporte rien).</td></tr>
