...
## Dead code & deprecated methods <span class="k-status k-status--orange">Clarified</span>

<table class="k-ticket k-ticket--orange">
  <tbody>
    ...
    <tr><td><strong>Analysis.</strong> <ol><li><code>EntityMetadata</code>: fix the phantom docblock (<code>PropertyInterface[]</code> → <code>PropertyMetadata[]</code>), drop <code>getTypeOfField()</code> and <code>getTypeOfAssociation()</code>.</li><li><code>TemplateResolver</code>: use <code>$property-&gt;type-&gt;value</code> instead of the wrappers.</li><li>Grep <code>src/</code> and <code>tests/</code> for remaining references.</li><li>Validate: <code>make all-fix</code> then <code>make all-check</code> green; template resolution &amp; formatting tests pass unchanged.</li></ol></td></tr>
  </tbody>
</table>
